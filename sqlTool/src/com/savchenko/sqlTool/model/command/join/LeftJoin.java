...
public class LeftJoin extends Join {
    ...
    public Pair<LazyTable, Integer> run(LazyTable lazyTable,
        ...
        var remainder = ModelUtils.getIndexedData(lazyTable.dataStream())
                .filter(pair -> !result.getMiddle().contains(pair.getLeft()))
                .map(pair -> ListUtils.union(pair.getRight(), ModelUtils.emptyRow(joinedLazyTable)));

        var dataStream = Stream.concat(result.getLeft(), remainder);

        return Pair.of(new LazyTable(null, columns, dataStream, lazyTable.externalRow()), remainder.toList().size());
    }
